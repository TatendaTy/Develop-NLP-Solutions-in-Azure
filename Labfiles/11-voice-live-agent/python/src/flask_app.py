...
class BasicVoiceAssistant:
    ...
    def __init__(
        ...
        self._response_cancelled = False # handles user interruptions
        self._stopping = False  # signals graceful shutdown
        self.state_callback = state_callback or (lambda *_: None) # no-op
    ...
    async def _handle_event(self, event, conn, verbose=False):
        """Handle voice live event with clear seperation by event type."""
    ...
    async def _handle_speech_started(self, conn):
        ...
        try:
            ...
            if current_state in {"assistant_speaking", "processing"}:
                ...
                _broadcast({"type": "log", "level": "info", "msg": f"Interrupted assistant during {current_state}"})
            ...
            _broadcast({"type": "log", "level": "debug", "msg": f"Exception in speech handler: {e}"})
    ...
    async def _handle_audio_delta(self, event):
        ...
        if assistant_state.get("state") != "assistant_speaking":
            self.state_callback("assistant_speaking", "Assistant is speaking...")
    ...
    async def _handle_audio_done(self):
        ...
        self._response_cancelled = False
        self.state_callback("ready", "Assistant finished speaking. You can speak now.")
...
def _run_assistant_bg():
    ...
    try:
        ...
        # Normalize common env value issues inline: trim whitespace and surrounding quotes
        endpoint = cast(str, endpoint).strip().strip('\"\'')
        model = cast(str, model).strip().strip('\"\'')
